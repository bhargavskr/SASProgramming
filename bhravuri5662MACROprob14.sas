 /*
           ...
           Problem Identification: MACRO Assignment 7 Problem 14
           Date:3/21/2015
           Purpose:  To use macros with positional parameters and print the all the sales of a particular product category
...
* noncorrelated subquery is a query with in another query and "any" is used so that the comparision is true if it is true for any of
the values that subquery returns;
* summary function sum() calculates the sum of Total retail prices across rows;
options mprint symbolgen;

%macro sales(prod_categ,start_date,end_date);


proc sql;
title "The sales for &prod_categ between &start_date and &end_date";
select sum(Total_Retail_Price) from orion.order_fact where product_id=any(select product_id from
...
%sales(Swim Sports,01MAR04,31DEC05)
%sales(Shoes,.,31DEC05)
